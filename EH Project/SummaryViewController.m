...
#define BODY_FONT            [UIFont fontWithName:@"HelveticaNeue" size:14]
...
{
    [super viewDidLoad];
    _summaryTitle.alpha = 0.0;
    [self.navigationController.navigationBar setBackgroundImage:[UIImage imageNamed: @"red_nav_bar.png"]
                                                 forBarMetrics:UIBarMetricsDefault];
    [self.summaryToolbar setBackgroundImage:[UIImage imageNamed:@"red_nav_bar.png"] forToolbarPosition:UIToolbarPositionAny barMetrics:UIBarMetricsDefault];
...
//
    ...
    [UIView animateWithDuration:0.9
                     ...
                     animations:^{
                      ...
                      //   _summaryTitle.center= CGPointMake(self.view.center.x,_summaryTitle.center.y+140);
                         _summaryTitle.alpha = 1.0;
}
...
- (void)loadIntroSection {
   ...
   // [dateFormat setDateFormat:@"MM:dd:yy zzz"];
    ...
    MGLineStyled
    *header = [MGLineStyled lineWithLeft:@"Upcoming events" right:dateAsString size:ROW_SIZE];
...
for (int i = 1; i <= 9; i++) {
    ...
    *layoutLine = [MGLineStyled lineWithLeft:@"iOS Lecture about walled gardens" right:arrow
                                        size:ROW_SIZE];
...
- (void)loadNewsFeed {
    ...
    MGLineStyled
    *header = [MGLineStyled lineWithLeft:@"News Feed" right:nil size:ROW_SIZE];
    header.font = HEADER_FONT;
    [menu.topLines addObject:header];
    
    // layout menu line
    MGLineStyled
    *layoutLine = [MGLineStyled lineWithLeft:@"Some Random crap about something" right:arrow
                                        size:ROW_SIZE];
    [menu.topLines addObject:layoutLine];
    ...
    // convenience features menu line
    MGLineStyled
    *conviniLine = [MGLineStyled lineWithLeft:@"Another groundbreakingly-important news item."
                                        right:arrow size:ROW_SIZE];
    [menu.topLines addObject:conviniLine];
